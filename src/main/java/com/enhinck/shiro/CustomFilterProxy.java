...
public class CustomFilterProxy extends DelegatingFilterProxy {
	private static Logger logger = LoggerFactory.getLogger(CustomFilterProxy.class);
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        logger.debug("进入自定义过滤器代理(CustomFilterProxy)......请求url:" + uri);
        if (excludeUrls != null && !excludeUrls.isEmpty()) {
            ...
            for (String path : excludeUrls) {
                ...
                if (uri.matches(path)) {
                	 logger.debug("跳过ShiroFilter,直接返回请求数据......");
                    filterChain.doFilter(request, response);
                    break;
                }
            }
        }
        logger.debug("进入 shiroFilter 代理器(DelegatingFilterProxy)......");
